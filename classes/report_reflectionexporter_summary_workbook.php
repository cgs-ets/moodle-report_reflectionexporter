...
namespace report_reflectionexporter\summary;
...
function report_reflectionexporter_summary_workbook($context, $studentsdata, $tempdir, $course) {

    require_capability('mod/assign:grade', $context);

    $filename       = $course->shortname . '.xls';
    ...
    $pos            = report_reflectionexporter_add_header($workbook,
                                                           $sheet,
                                                           $course->fullname,
                                                           get_string('extendedessayheader', 'report_reflectionexporter')
                                                        );
...
function report_reflectionexporter_add_info_header(MoodleExcelWorkbook $workbook,
    ...
    $sheet->write_string(5, $pos++, get_string('ibcode', 'report_reflectionexporter'), $format2);
    $sheet->set_column($pos - 1, $pos, 30); // Set column widths to 20.
    $sheet->write_string(5, $pos++, get_string('missingactivities', 'report_reflectionexporter'), $format2);
...
function report_reflectionexporter_set_students_rows (MoodleExcelWorksheet $sheet, $studentsdata) {
    ...
    foreach ($studentsdata as $studentdata) {
        ...
        $sheet->write_string($row, $col++, $studentdata->id, $format);
        $sheet->write_string($row, $col++, $studentdata->missing, $format);
    }
