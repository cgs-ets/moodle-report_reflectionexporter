...
define(['jquery', 'core/notification', 'core/str', 'core/ajax'], function (
  $,
  notification,
  str,
  ajax,
  ...
  viewingNavigation.prototype._usersLoaded = function (users) {
    ...
    //If the teachers comments are not empty, the first user is last user.
    ...
    var firstUser =
      withcomment == 0
        ? this._users[0].id
        : this._users[this._users.length - 1].id;
    $('[data-region="viewer-navigation-panel"]').attr(
      'data-first-userid',
      firstUser,
    );
    $('[data-region="viewer-navigation-panel"]').attr(
